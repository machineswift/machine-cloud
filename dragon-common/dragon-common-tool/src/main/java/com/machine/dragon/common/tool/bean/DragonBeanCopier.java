 ...
 * </p>
 */
public abstract class DragonBeanCopier {
	private static final Type CONVERTER = TypeUtils.parseType("org.springframework.cglib.core.Converter");
	private static final Type BEAN_COPIER = TypeUtils.parseType(DragonBeanCopier.class.getName());
	...
	/**
	 * The map to store {@link DragonBeanCopier} of source type and class type for copy.
	 */
	private static final ConcurrentMap<DragonBeanCopierKey, DragonBeanCopier> BEAN_COPIER_MAP = new ConcurrentHashMap<>();

	public static DragonBeanCopier create(Class source, Class target, boolean useConverter) {
		return DragonBeanCopier.create(source, target, useConverter, false);
	}

	public static DragonBeanCopier create(Class source, Class target, boolean useConverter, boolean nonNull) {
		DragonBeanCopierKey copierKey = new DragonBeanCopierKey(source, target, useConverter, nonNull);
	...
	public static class Generator extends AbstractClassGenerator {
		private static final Source SOURCE = new Source(DragonBeanCopier.class.getName());
		...
		@Override
		public DragonBeanCopier create(Object key) {
			return (DragonBeanCopier) super.create(key);
		}
		...
		public void generateClass(ClassVisitor v) {
			...
			for (PropertyDescriptor setter : setters) {
				...
				if (targetIgnoreCopy != null) {
					...
					if (DragonStringUtil.isNotBlank(aliasTargetPropName)) {
				...
				// 判断类型是否一致，包括 包装类型
				if (DragonClassUtil.isAssignable(setterPropertyType, getterPropertyType)) {
					// 2018.12.27 by L.cm 支持链式 bean
		...
		protected Object firstInstance(Class type) {
			return DragonBeanUtil.newInstance(type);
		}
