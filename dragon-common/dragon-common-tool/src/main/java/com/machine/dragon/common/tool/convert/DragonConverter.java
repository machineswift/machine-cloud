...
import com.machine.dragon.common.tool.bean.ReflectUtil;
import com.machine.dragon.common.tool.function.CheckedFunction;
import com.machine.dragon.common.tool.lamba.Unchecked;
...
@Slf4j
@AllArgsConstructor
public class DragonConverter implements Converter {
    ...
    public Object convert(Object value, Class target, final Object fieldName) {
        ...
        // 类型一样，不需要转换
        if (DragonClassUtil.isAssignableValue(target, value)) {
            return value;
        }

        TypeDescriptor targetDescriptor = DragonConverter.getTypeDescriptor(targetClazz, (String) fieldName);
        // 1. 判断 sourceClazz 为 Map
        if (Map.class.isAssignableFrom(sourceClazz)) {
            return DragonConvertUtil.convert(value, targetDescriptor);
        } else {
            TypeDescriptor sourceDescriptor = DragonConverter.getTypeDescriptor(sourceClazz, (String) fieldName);
            return DragonConvertUtil.convert(value, sourceDescriptor, targetDescriptor);
        }
    }
